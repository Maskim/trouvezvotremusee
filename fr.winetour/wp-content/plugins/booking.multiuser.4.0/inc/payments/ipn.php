...
try {
   ...
   if ($paypal_ipn_is_send_error_email == 'On')  {
        ...
        mail($paypal_ipn_error_email , __('Error IPN','wpdev-booking'), $e->getMessage() );
   }
   exit(0);
}
...
if ($verified) {
   ...
   if ($is_ok_tranzaction) {  // All checking is PASS
       ...
       if ($payment_status == 'Completed')
               check_auto_approve_or_cancell($my_boooking_id, true, 'paypal');

       if ( ($payment_status == 'Denied') ||  ($payment_status == 'Failed' ) ||  ($payment_status == 'Refunded') )
               check_auto_approve_or_cancell($my_boooking_id, false, 'paypal');
       ...
       if ($paypal_ipn_is_send_verified_email == 'On')  {
            ...
            mail($paypal_ipn_verified_email,  __('Verified IPN','wpdev-booking'), $listener->getTextReport());
       }
   ...
   if ($paypal_ipn_is_send_invalid_email == 'On')  {
        ...
        mail($paypal_ipn_invalid_email ,   __('Invalid IPN','wpdev-booking'), $listener->getTextReport());
   }
