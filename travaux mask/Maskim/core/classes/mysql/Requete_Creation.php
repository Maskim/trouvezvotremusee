<?php
class Requete_Creation
{
	private $connexion;
	
	public function Requete_Creation($host,$bdd,$login,$pass)
	{
		$this->host = $host;
		$this->bdd = $bdd;
		$this->login = $login;
	...
	public function query($schema)
	{
		...
		$req = "CREATE DATABASE ".$this->bdd;
		$sql = mysql_query($req, $this->connexion) or die(mysql_error());
		system("mysql.exe -h ".$this->host." -u".$this->login." -p".$this->pass." ".$this->bdd." < ".$schema);
	}
